...
def random_crop_and_flip(batch_data, padding_size):
    ...
    '''
    cropped_batch = np.zeros(len(batch_data) * IMG_HEIGHT_16 * IMG_WIDTH_16 * IMG_DEPTH).reshape(
        len(batch_data), IMG_HEIGHT_16, IMG_WIDTH_16, IMG_DEPTH)

    for i in range(len(batch_data)):
        x_offset = np.random.randint(low=0, high=2 * padding_size, size=1)[0]
        y_offset = np.random.randint(low=0, high=2 * padding_size, size=1)[0]
        cropped_batch[i, ...] = batch_data[i, ...][x_offset:x_offset + IMG_HEIGHT_16,
                                y_offset:y_offset + IMG_WIDTH_16, :]
