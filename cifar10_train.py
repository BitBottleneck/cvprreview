...
class Train(object):
    ...
    def placeholders(self):
        ...
        self.image_placeholder = tf.placeholder(dtype=tf.float32,
                                                shape=[FLAGS.train_batch_size, IMG_HEIGHT_16,
                                                       IMG_WIDTH_16, IMG_DEPTH])
        ...
        self.vali_image_placeholder = tf.placeholder(dtype=tf.float32, shape=[FLAGS.validation_batch_size,
                                                                              IMG_HEIGHT_16, IMG_WIDTH_16, IMG_DEPTH])
        ...
        for step in range(FLAGS.train_steps):
            ...
            if step % FLAGS.report_freq == 0:
                ...
                print(format_str % (datetime.now(), step, train_loss_value, examples_per_sec,
                                    sec_per_batch))
                # print('Train top1 accuracy = ', np.around(1.0-train_error_value, decimals=4))
                # print('Validation top1 accuracy = ', np.around(1.0-validation_error_value, decimals=4))
                print('Train top1 accuracy = %.2f%%' % ((1.0 - train_error_value) * 100))
                print('Validation top1 accuracy = %.2f%%' % ((1.0 - validation_error_value) * 100))
            ...
            if (step % 1000 == 0 and step != 0) or (step + 1) == FLAGS.train_steps:
    ...
    def cus_test(self):
        ...
        vali_logits = inference(self.vali_image_placeholder, FLAGS.num_residual_blocks, reuse=tf.AUTO_REUSE)
        ...
        validation_loss_value, validation_error_value = self.full_validation(loss=vali_loss,
                                                                            top1_error=vali_top1_error,
                                                                            vali_data=vali_data,
                                                                            vali_labels=vali_labels,
                                                                            session=sess,
                                                                            batch_data=train_batch_data,
